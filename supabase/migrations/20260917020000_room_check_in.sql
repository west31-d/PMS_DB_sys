-- Preserve per-room arrival state for reservations containing multiple rooms.
ALTER TABLE public.reservation_room ADD COLUMN stay_status text NOT NULL DEFAULT '예약'
  CHECK (stay_status IN ('예약', '재실', '퇴실', '취소'));
UPDATE public.reservation_room rr SET stay_status = r.status
FROM public.reservation r WHERE r.reservation_id = rr.reservation_id;

CREATE FUNCTION public.pms_check_in_room(p_reservation_room_id bigint)
...
BEGIN
  ...
  IF v_room.housekeeping_status = '재실' OR EXISTS (
    ...
    WHERE rr.room_id = v_line.room_id AND rr.reservation_room_id <> p_reservation_room_id
      AND rr.stay_status = '재실' AND r.status = '재실'
  ...
  UPDATE public.room SET housekeeping_status = '재실' WHERE room_id = v_line.room_id;
