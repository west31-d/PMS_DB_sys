...
## 개발자 페이지 제거
사용자 요청으로 개발자 메뉴와 PT/DB_ERD/Source/WBS/Feedback 화면을 제거했습니다. 현재 운영 메뉴는 18개이며 기존 개발자 URL은 찾을 수 없는 페이지로 처리합니다. 사이드바 전환 효과도 함께 반영합니다. 위의 개발 메뉴 설명은 이전 작업 이력입니다.

## 실 입실 목록 구현

- CheckedIn 페이지: 왼쪽 조건, 오른쪽 결과 표. 기본 입실일 오늘~오늘, 예약 상태 재실 고정.
- 입실일 범위·고객·객실·예약번호·거래처·객실타입 필터, 조회/초기화, CSV, 상세 패널.
- checkInReport: 필터, 박수, 예약별 객실료/서비스 합계. 입실시간·요금타입 임의 생성 없음.
- live 모드의 Supabase 예약 조회에도 property_id, status=재실, check_in gte/lte 적용.
- 동기화 모드는 객실/타입만 있으므로 예약을 가짜로 채우지 않음.
- 빌드 및 단위 테스트 11개, 브라우저 테스트 7개 통과.

## 업무 다중 탭과 메뉴 자동 접기

- 페이지 방문 시 헤더 아래 업무 탭 추가. 같은 경로는 기존 탭으로 이동하고 중복 생성하지 않음.
- 대시보드 기본 탭 유지, 다른 탭은 개별 닫기 가능. 활성 탭 닫으면 왼쪽 인접 탭으로 이동.
- 탭 전환 시 페이지 컴포넌트를 유지하여 검색·필터·입력·페이지 이동 상태 보존. 조회 기준일도 탭별로 관리.
- 브라우저 새로고침 시 탭은 초기화. 호텔/데이터 모드 변경 시 화면 상태 초기화.
- 왼쪽 메뉴 선택 시 데스크톱은 아이콘 상태로 접히고 모바일 메뉴는 닫힘. 헤더 메뉴 버튼으로 다시 열기.
- 기존 부드러운 메뉴 전환 유지. 좌우 방향키/Home/End/Delete로 탭 이동/닫기 지원.
- 빌드 및 브라우저 테스트 8개 통과.

## 화면 설명 정리
동기화 상태줄, 페이지 부제, 정보 배너, 반복 설명과 하단 안내를 제거했습니다. 호텔 선택은 제목 옆으로 이동했습니다. 데이터 처리와 기능은 유지하며 오류·빈 결과 상태 및 데모 표시만 간결하게 남깁니다.
