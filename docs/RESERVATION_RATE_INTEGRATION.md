...
- 고객명 직접 입력. 기존/신규 구분이나 고객 검색 없이 입력한 이름으로 고객 레코드를 생성한다. 동명이인을 이름만으로 자동 병합하지 않는다.
...
| 입력 | 실제 저장 위치 |
| --- | --- |
| 고객명 | `customer.customer_name` 생성 후 예약에 해당 ID 연결 |
...
- `PMS_TEST_LIVE=true` Playwright: 화면에서 요금을 직접 입력하여 810,000원 합계, 거래처 변경 시 요금 유지, 기간 변경 재계산, 고객명 저장 payload, 호수 배정, 모의 RPC 실패·재시도·목록 반영을 검증한다.
